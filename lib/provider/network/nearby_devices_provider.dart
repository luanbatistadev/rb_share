...
import 'package:rb_share/model/state/nearby_devices_state.dart';
...
import 'package:rb_share/provider/network/multicast_provider.dart';
import 'package:rb_share/provider/network/targeted_discovery_provider.dart';
import 'package:rb_share/util/task_runner.dart';
import 'package:refena_flutter/refena_flutter.dart';

final _logger = Logger('NearbyDevices');

/// This provider is responsible for:
/// - Scanning the network for other RBShare instances
...
class NearbyDevicesService extends ReduxNotifier<NearbyDevicesState> {
  ...
  Future<Device?> _doRequest(String currentIp, int port, bool https) async {
    ...
    if (device != null) {
      _discoveryLogs
          .addLog('[DISCOVER/TCP] ${device.alias} (${device.ip}, model: ${device.deviceModel})');
    }
...
class StartMulticastListener extends AsyncReduxAction<NearbyDevicesService, NearbyDevicesState> {
  ...
  Future<NearbyDevicesState> reduce() async {
    await for (final device in notifier._multicastService.state.startListener()) {
      ...
      notifier._discoveryLogs
          .addLog('[DISCOVER/UDP] ${device.alias} (${device.ip}, model: ${device.deviceModel})');
    }
    return state;
  }
}
...
class RegisterDeviceAction extends AsyncReduxAction<NearbyDevicesService, NearbyDevicesState> {
  ...
  Future<NearbyDevicesState> reduce() async {
    ...
    if (favoriteDevice != null && !favoriteDevice.customAlias) {
      ...
      await external(notifier._favoriteService)
          .dispatchAsync(UpdateFavoriteAction(favoriteDevice.copyWith(alias: device.alias)));
    }
    return state.copyWith(
...
/// It just sends an announcement which will cause a response on every other RBShare member of the network.
