...
import 'package:rb_share/model/state/server/receiving_file.dart';
import 'package:rb_share/pages/home_page.dart';
import 'package:rb_share/pages/progress_page.dart';
import 'package:rb_share/pages/receive_page.dart';
...
import 'package:rb_share/provider/logging/discovery_logs_provider.dart';
...
import 'package:rb_share/provider/network/server/server_utils.dart';
import 'package:rb_share/provider/progress_provider.dart';
import 'package:rb_share/provider/receive_history_provider.dart';
import 'package:rb_share/provider/settings_provider.dart';
import 'package:rb_share/util/api_route_builder.dart';
import 'package:rb_share/util/native/directories.dart';
import 'package:rb_share/util/native/file_saver.dart';
  ...
  void installRoutes({
    ...
    router.post(ApiRoute.register.v1, (Request request) async {
      return _registerHandler(
          request: request, alias: alias, port: port, https: https, fingerprint: fingerprint,);
    });
  ...
  Future<Response> _uploadHandler({
    ...
    try {
      ...
      final destinationPath = await digestFilePathAndPrepareDirectory(
        parentDirectory:
            saveToGallery ? receiveState.cacheDirectory : receiveState.destinationDirectory,
    ...
    if (receiveSession != null) {
      ...
      if (v2) {
        ...
        final selectedSession =
            sendSessions.values.firstWhereOrNull((s) => s.remoteSessionId == sessionId);
