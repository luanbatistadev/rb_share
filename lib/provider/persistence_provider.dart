...
import 'package:rb_share/util/alias_generator.dart';
import 'package:rb_share/util/native/platform_check.dart';
import 'package:rb_share/util/security_helper.dart';
import 'package:rb_share/util/shared_preferences_portable.dart';
...
class PersistenceService {
  ...
  static Future<PersistenceService> initialize(DynamicColors? dynamicColors) async {
    ...
    if (prefs.getString(_colorKey) == null) {
      ...
      final colorMode =
          supported.firstWhereOrNull((color) => color.name == prefs.getString(_colorKey));
      if (colorMode == null) {
        await _initColorSetting(prefs, supportsDynamicColors);
      }
    }
  ...
  SendMode getSendMode() {
    return SendMode.values.firstWhereOrNull((m) => m.name == _prefs.getString(_sendMode)) ??
        SendMode.single;
  }
