...
import 'package:rb_share/util/determine_image_type.dart';
...
import 'package:rb_share/util/sleep.dart';
import 'package:rb_share/util/ui/asset_picker_translated_text_delegate.dart';
...
Future<void> _pickMedia(BuildContext context, Ref ref) async {
  ...
  final List<AssetEntity>? result = await AssetPicker.pickAssets(
    context,
    pickerConfig:
        const AssetPickerConfig(maxAssets: 999, textDelegate: TranslatedAssetPickerTextDelegate()),
  );
...
Future<void> _pickText(BuildContext context, Ref ref) async {
  final result =
      await showDialog<String>(context: context, builder: (_) => const MessageInputDialog());
  if (result != null) {
