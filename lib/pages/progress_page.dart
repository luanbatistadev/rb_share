...
import 'package:rb_share/util/file_speed_helper.dart';
import 'package:rb_share/util/native/open_file.dart';
import 'package:rb_share/util/native/open_folder.dart';
...
import 'package:rb_share/widget/custom_progress_bar.dart';
import 'package:rb_share/widget/dialogs/cancel_session_dialog.dart';
...
class _ProgressPageState extends State<ProgressPage> with Refena {
  ...
  Future<bool> _onWillPop() async {
    ...
    if (!widget.closeSessionOnClose &&
        (status == SessionStatus.sending || status == SessionStatus.finishedWithErrors)) {
  ...
  Widget build(BuildContext context) {
    ...
    final currBytes = _files.fold<int>(
        ...
        (prev, curr) =>
            prev +
            ((progressNotifier.getProgress(sessionId: widget.sessionId, fileId: curr.id) *
                    curr.size)
                .round()),);
    ...
    if (startTime != null && currBytes >= 500 * 1024) {
      ...
      if (now - _lastRemainingTimeUpdate >= 1000) {
        _remainingTime = getRemainingTime(
            bytesPerSeconds: speedInBytes, remainingBytes: _totalBytes - currBytes,);
          ...
          children: [
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final fileStatus =
                    receiveSession?.files[file.id]?.status ?? sendSession!.files[file.id]!.status;
                                    ...
                                    Flexible(
                                      child: Text(
                                        savedToGallery
                                            ...
                                            : fileStatus.label,
                                        style: TextStyle(
                                            color: fileStatus.getColor(context), height: 1,),
                                      ),
                                    ),
                                    if (errorMessage != null) ...[
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          AnimatedCrossFade(
                            crossFadeState:
                                _advanced ? CrossFadeState.showSecond : CrossFadeState.showFirst,
